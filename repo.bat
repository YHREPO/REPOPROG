...
echo.
echo.
if not exist "%systemdrive%\RepoTEMP\bin\chn.txt" goto S_CHN
...
if exist "settings.ini" del /q settings.ini
if not exist "%systemdrive%\RepoTEMP\bin\wget.exe" goto DWLD_F
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/yhrepo/master/settings.ini
cd %systemdrive%\RepoTEMP\bin
echo CHANNEL >>%systemdrive%\RepoTEMP\bin\chn.txt
goto DWLD_F

:LIC
cls
mode con cols=120 lines=900
cd %systemdrive%\RepoTEMP
set LCD=Y
...
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/yhrepo/master/settings.ini
:DWLD_2
for /f "tokens=2 delims==" %%u in ('type "%systemdrive%\RepoTEMP\settings.ini" ^|findstr /bi "CH="') do (set "CHN=%%u")
if not "%CHN%"=="1" goto DWLD_l
...
:DWLD_l
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/yhrepo/master/list%CHN%.txt
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/yhrepo/master/link%CHN%.encrypted
cd %systemdrive%\RepoTEMP
rename list%CHN%.txt list.txt
rename link%CHN%.encrypted link.encrypted
if not exist "%systemdrive%\RepoTEMP\settings.ini" goto ER2
if not exist "%systemdrive%\RepoTEMP\link.encrypted" goto ER2
if not exist "%systemdrive%\RepoTEMP\list.txt" goto ER2
if not exist "%systemdrive%\RepoTEMP\Tool.bat" goto ER2
...
for /f "tokens=2 delims==" %%g in ('type "%systemdrive%\RepoTEMP\settings.ini" ^|findstr /bi "CH="') do (set "CHN=%%g")
for /f "tokens=2 delims=$" %%k in ('type "%systemdrive%\RepoTEMP\Tool.bat" ^|findstr /bi "##VER"') do (set "TVR=%%k")
if not "%TVR%" == "1.1" goto DWLD_T
if "%DWLD_F%"=="1" set DWLD_F=%userprofile%\Downloads
if not exist "%systemdrive%\RepoTEMP\list.txt" set LIST=리스트가 발견되지 않았습니다.
echo =================================================
echo YH Repository - MAIN [SERVER : #%CHN%]
echo =================================================
echo.
...
echo.
echo.               
echo 아래에 원하는 파일 링크를 입력 후 
echo ENTER 키를 눌러 주세요.
echo.
echo.
echo.
echo EX) https://github.com/yhrepo/example.exe
echo.
echo.
echo 메인으로 나가시려면 exit를 입력해 주세요
set /p "LNK=>"
if "%LNK%"=="" goto ER_m5
if "%LNK%"=="exit" goto MAIN
if "%LNK%"=="EXIT" goto MAIN
goto FLD_2
...
echo.
echo 잠시 기다려 주세요 . . .
call %systemdrive%\RepoTEMP\Tool.bat %SMD% QnlZSFJFUE8=
cd %systemdrive%\RepoTEMP
...
for /f "tokens=2 delims==" %%g in ('type "%systemdrive%\RepoTEMP\settings.ini" ^|findstr /bi "DF="') do (set "DWLD_F=%%g")
if "%DWLD_F%"=="1" set DWLD_F=%userprofile%\Downloads
if exist "%systemdrive%\RepoTEMP\Decrypted.txt" del %systemdrive%\RepoTEMP\Decrypted.txt
cls
echo =================================================
echo YH Repository - DOWNLOADING FILE
echo =================================================
...
echo.
echo 잠시 기다려 주세요 . . .
bin\wget.exe -q -nc -P %DWLD_F% --no-hsts %LNK% ||goto ER_dwld
del /q %systemdrive%\RepoTEMP\Decrypted.txt
...
cls
echo =================================================
echo YH Repository - DOWNLOAD Failed
echo =================================================
echo.
echo 파일 다운로드
echo.
...
echo.
echo.
echo 파일을 다운로드 하는 도중 오류가 발생하였습니다.
echo.
echo 아무키나 누르면 메인으로 나갑니다.
pause>nul
goto MAIN
...
:UPD_D
cd %systemdrive%\RepoTEMP
echo 1.4 >>%systemdrive%\RepoTEMP\db.txt
goto CHKUPD
...
for /f "tokens=2 delims==" %%u in ('type "%systemdrive%\RepoTEMP\settings.ini" ^|findstr /bi "CH="') do (set "SV_CH=%%u")
set RP_SV=#%SV_CH%
cls
echo =================================================
...
if not exist "%systemdrive%\RepoTEMP\bin\chn.txt" goto S_DWLD
for /f "tokens=2 delims==" %%g in ('type "%systemdrive%\RepoTEMP\settings.ini" ^|findstr /bi "CH="') do (set "CHN=%%g")
:S_DEV
set menu=
cls
...
:DWLD_F
for /f "tokens=6 delims=[]. " %%G in ('ver') do set winbuild=%%G
if "%winbuild%"=="7601" goto DWLD_w7
if "%winbuild%"=="7600" goto DWLD_w7
:DWLD_F3
if exist "%systemdrive%\RepoTEMP\bin\wget.exe" goto DWLD_F2
cls
echo =================================================
echo YH Repository - DOWNLOADING FILES
echo =================================================
...
:DWLD_F2
cd %systemdrive%\RepoTEMP
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/REPOPROG/main/Tool.bat
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/yhrepo/master/settings.ini
...
:DWLD_T
cd %systemdrive%\RepoTEMP
if exist "Tool.bat" del /q Tool.bat
cls
echo =================================================
echo YH Repository - UPDATING TOOL.BAT
echo =================================================
echo.
echo Tool.bat 업데이트
echo.               
echo "Tool.bat" 파일 업데이트를 진행합니다.	          
echo.
echo 이 작업은 한 번만 진행됩니다.
echo.
echo.
echo 버전 1.0 -^> 버전 1.1
echo. 
...
if not exist "%systemdrive%\RepoTEMP\bin\wget.exe" goto ER2
bin\wget.exe -q -nc -P %systemdrive%\RepoTEMP --no-hsts https://raw.githubusercontent.com/YHREPO/REPOPROG/main/Tool.bat
if not exist "%systemdrive%\RepoTEMP\Tool.bat" goto ER_T
goto DWLD_T_D
...
cls
echo =================================================
echo YH Repository - Completed Update
echo =================================================
echo.
echo Tool.bat 업데이트
echo.               
...
echo 업데이트가 완료되었습니다.
echo.
echo YH Repository를 다시 실행하여 주세요.
echo.
echo. 
echo.
echo 아무키나 누르면 종료됩니다.
...
cls
echo =================================================
echo YH Repository - CAUTION
echo =================================================
echo.
echo 주의
echo.               
echo 운영체제의 버전이 너무 낮아
echo.
echo 파일을 다운로드 중 오류가 발생할 수 있습니다.
echo.
echo 계속 다운로드를 진행하시겠습니까?
echo.
echo [Y,N]
echo.
echo 입력후 ENTER 키를 눌러 주세요.
set /p error=
if /i "%error%" EQU "Y" goto :DWLD_F3
if /i "%error%" EQU "y" goto :DWLD_F3
